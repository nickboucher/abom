...
def main() -> None:
    ...
    parser_graph.add_argument('-f', '--max-f', type=float, default=1/(2**14), help='Maximum false positive rate to graph.')
...
def graph_4D(stats: list[BFStats], out_dir: str, max_f: float) -> None:
    ...
    for m, X in M.items():
        filename = join(out_dir, f'4D/m-{m}.png')
...
def graph_optimals(stats: list[BFStats], out_dir: str, max_f: float, min_n: int) -> defaultdict[defaultdict[float]]:
    ...
    plt.title(f'Optimal $(m,n,k)$\nfor $f\leq{str(max_f).lstrip("0")}$ and $n\geq{min_n}$')
    plt.ylabel('Compressed Size (bytes)')
    fig.savefig(join(out_dir,'optimals.png'), dpi=300)
    plt.close()
    return optimals
...
def optimals_table(optimals: defaultdict[defaultdict[float]], out_dir: str, max_f: float, min_n: int, t: int) -> None:
    ...
    plt.title(f'Top {t} Optimal $(m,n,k,z)$\nfor $f\leq{str(max_f).lstrip("0")}$ and $n\geq{min_n}$', pad=20)
    fig.savefig(join(out_dir,'optimals_table.png'), bbox_inches='tight', dpi=300)
    plt.close()
